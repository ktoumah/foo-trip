...
init: ## Install the application
	make build start copy_env_file install_dependencies install_db fix_folder_permissions
...
app_exec_cmd: app_start  ## Execute commands in PHP container
	docker exec php_foo_trip_container bash -c "${OPT}"

db_exec_cmd: db_start  ## Execute commands in DB container
	docker exec mysql_foo_trip_container bash -c "${OPT}"
...
install_db:  ## Create database and schema
	...
	make db_exec_cmd OPT="mysql -u root -p'root' foo_trip_db -e 'SET FOREIGN_KEY_CHECKS = 0; source /data/foo_trip_db.sql; SET FOREIGN_KEY_CHECKS = 1;'"
...
sh_app: app_start ## Connect to PHP container
	docker exec -it php_foo_trip_container bash

sh_db: db_start ## Connect to Mysql container
	docker exec -it mysql_foo_trip_container bash

app_start:
	[ `docker ps -q -f name=php_foo_trip_container` ] || docker-compose up -d php_service

nginx_start:
	[ `docker ps -q -f name=nginx_foo_trip_container` ] || docker-compose up -d nginx_service

db_start:
	[ `docker ps -q -f name=mysql_foo_trip_container` ] || docker-compose up -d mysql_service_foo_trip
